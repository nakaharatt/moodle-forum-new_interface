...
function forum_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
    ...
    echo '<a href="'.$CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'" style="font-size:1.3em">'.$post->subject.'</a>';
    ...
    */
    ...
    if ($group !== -1) {  // Groups are active - group is a group data object or NULL
        if (!empty($group->picture) and empty($group->hidepicture)) {
            ...
            echo '<td class="group">';
            if ($canviewparticipants && $COURSE->groupmode) {
                //echo '<a href="'.$CFG->wwwroot.'/user/index.php?id='.$forum->course.'&amp;group='.$group->id.'">'.$group->name.'</a>'; //LinkText
...
// If we want paging
    ...
    if ($displayformat == 'header') {
        ...
        if ((!is_guest($context, $USER) && isloggedin()) && has_capability('mod/forum:viewdiscussion', $context)) {
            if (\mod_forum\subscriptions::is_subscribable($forum)) {
                echo '<th class="header discussionsubscription" scope="col">'.get_string('follow', 'forum');
                //echo forum_get_discussion_subscription_icon_preloaders();
                echo '</th>';
            }
        }
        //echo '<th class="header author" colspan="1" scope="col">'.get_string('follow', 'forum').'</th>';
        if ($groupmode > 0) {
